#!/usr/bin/env bash
# bwuni3.sh — ctun budget script: total Slurm job *wall-clock hours* for the
...
# Slurm accounting (sacct). bwUniCluster 3.0 (uc3) runs Slurm with accounting
# enabled, so the standard metric applies unchanged.
#
