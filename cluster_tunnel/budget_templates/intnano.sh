#!/usr/bin/env bash
# intnano.sh — ctun budget script: total Slurm job *wall-clock hours* for the
...
# Slurm accounting (sacct), which works on any cluster with sacct accounting
# enabled. Verify `sacct` reports Start/End on intnano; otherwise the logic is
# identical regardless of cluster.
#
